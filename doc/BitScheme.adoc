...
== Notes
=== Schema Construction Syntax

Need a different Syntax for Schema construction. As it is, we'd need to detect that the argument fails the 'naive' type-check, then recognize that the appropriate keys are present. Thats too complex and un-obvious.

What we really want is something like a map:
